...
std::vector<brave_wallet::EthereumChain> GetCustomChains(PrefService* prefs) {
...
std::vector<brave_wallet::EthereumChain> GetAllKnownNetworks() {
  std::vector<brave_wallet::EthereumChain> result;
  for (const auto& network : kKnownNetworks) {
    result.push_back(GetKnownNetwork(network.chain_id));
  }
  return result;
}

GURL GetNetworkURL(PrefService* prefs, const std::string& chain_id) {
  auto known_network = GetKnownNetwork(chain_id);
...
std::vector<EthereumChain> GetAllNetworks(PrefService* prefs) {
  std::vector<EthereumChain> networks = GetAllKnownNetworks();
  std::vector<EthereumChain> custom_chains = GetCustomChains(prefs);
